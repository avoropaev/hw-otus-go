...
{"Id":7,"Name":"","Username":"","Email":"aliquid_qui_ea@some.Browsedrive.gov","Phone":"","Password":"","Address":""}`
	...
	t.Run("find 'Browsedrive.gov'", func(t *testing.T) {
		result, err := GetDomainStat(bytes.NewBufferString(data), "Browsedrive.gov")
		require.NoError(t, err)
		require.Equal(t, DomainStat{
			"browsedrive.gov":      1,
			"some.browsedrive.gov": 1,
		}, result)
	})
	...
	t.Run("empty reader", func(t *testing.T) {
		result, err := GetDomainStat(strings.NewReader(""), "qwerty")
		require.NoError(t, err)
